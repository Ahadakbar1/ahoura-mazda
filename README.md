<div align="center">

# 🟢 مشکل بن شدن پنل ها رفع شد 🟢

## 🔶 یک اکانت فرعی کلودفلر بسازید (جیمیل فیک نزنید)
## 🔷 با ربات تلگرام یک پنل جدید بسازید (آموزش پایین هست)

</div>

<img  src="photos/dark2.png"  width="100%"  alt="Ahura Mazda Panel Dark Mode"  style="border-radius: 12px; margin-bottom: 15px;">

  
<img  src="photos/add.png"  width="100%"  alt="Ahura Mazda Panel Dark Mode"  style="border-radius: 12px; margin-bottom: 15px;">

  

<table  width="100%">

<tr>

<td width="50%" valign="middle" align="center">

<img  src="photos/updater.png"  width="100%"  alt="Ahura Mazda Panel Status"  style="border-radius: 12px;">

</td>

<td width="50%" valign="middle" align="center">

<img  src="photos/deployer.png"  width="100%"  alt="Ahura Mazda Panel Interface"  style="border-radius: 12px;">

</td>

</tr>

<tr>

<td width="50%" valign="middle" align="center">

<img  src="photos/bot.jpg"  width="100%"  alt="Ahura Mazda Panel Status"  style="border-radius: 12px;">

</td>

<td width="50%" valign="middle" align="center">

<img  src="photos/status.png"  width="100%"  alt="Ahura Mazda Panel Dark Mode"  style="border-radius: 12px;">

</td>

</tr>

</table>

  

<div  align="center">

<h1>⚡ پنل اهورا مزدا کلودفلر (Ahura Mazda Panel)</h1>

<p>

<a href="https://github.com/Ahadakbar1/ahura-mazda">

<img  src="https://img.shields.io/badge/Version-1.8.13-0052CC?style=for-the-badge&logo=semver&logoColor=white"  alt="Version">

</a>

<a href="https://cloudflare.com">

<img  src="https://img.shields.io/badge/Platform-Cloudflare_Workers-F38020?style=for-the-badge&logo=cloudflare&logoColor=white"  alt="Platform">

</a>

<a href="https://t.me/PANEL_ZEUS">

<img  src="https://img.shields.io/badge/Developer-PANEL_ZEUS-00792D?style=for-the-badge&logo=telegram&logoColor=white"  alt="Developer">

</a>

</p>

</div>

  

---

  

# ⚡️ قابلیت های پنل

  
  

🌐 آیپی و لوکیشن ثابت برای هر کاربر.

👥 مدیریت کاربران بر اساس حجم، زمان، ریکوئست و دستگاه‌های همزمان.

♻️ ریست خودکار حجم و ریکوئست بر اساس زمان 

🛠 عملیات گروهی برای ویرایش، حذف و ریست کاربران.

🛡 پشتیبانی از Fragment و شبیه‌ساز Fingerprint برای عبور از فیلترینگ.

📱 رابط کاربری مدرن (Tailwind CSS) با پشتیبانی از دارک‌مود و موبایل.

🛑 مسدودساز هوشمند سایت‌های پورنوگرافی و تبلیغات از طریق DNS.

📡 دریافت خودکار آی‌پی تمیز و امکان تنظیم پروکسی سفارشی .

🌐 تغییر خودکار آیپی های تمیز در زمان دلخواه

📊 مانیتورینگ زنده ریکوئست‌های کلودفلر برای جلوگیری از مسدودی.

🔗 تولید لینک ساب و QR کد و صفحه وضعیت اختصاصی هر کاربر.

🔄 آپدیت خودکارهسته پنل بدون از دست رفتن اطلاعات دیتابیس.

🗄 امکان بک‌آپ‌گیری کامل (Export/Import) اطلاعات در قالب JSON.

🚀 ساخت کامل پنل، ساب‌دامین و دیتابیس تنها با یک کلیک از ربات تلگرام.

🤖 مدیریت همزمان چندین اکانت کلودفلر، آپدیت پنل و بازیابی رمز عبور از طریق ربات تلگرام.

  

---

  

# 🚀 راهنمای نصب و استقرار سریع



<div  align="center">

<a  href="https://dash.cloudflare.com/"  target="_blank">

<img src="https://img.shields.io/badge/Cloudflare_Dashboard-Login-f38020?style=for-the-badge&logo=cloudflare&logoColor=white" alt="Cloudflare Dashboard" height="40">

</a>
<div align="center">
ابتدا وارد داشبورد کلودفلر شوید، با جیمیل خود وارد حساب کاربری شده و ایمیلتان را تایید کنید و سپس از یکی از روش‌های زیر ادامه دهید.
</div>

---

<a  href="https://t.me/AHURA_MAZDA_BOT"  target="_blank">

<img src="https://img.shields.io/badge/Zeus_Telegram_Bot-Start_Bot-0088cc?style=for-the-badge&logo=telegram&logoColor=white" alt="Zeus Telegram Bot" height="40">

</a>

</div>

<br>

  

### 🤖 روش اول: نصب از طریق ربات تلگرام (پیشنهادی)

1. 🌐 وارد **[ربات تلگرام زئوس](https://t.me/AHURA_MAZDA_BOT)** شوید و آن را `Start` کنید.

2. 👤 در منوی اصلی روی گزینه **«➕ ثبت اکانت کلودفلر»** کلیک کنید.

3. 🔗 دکمه شیشه‌ای **«🔑 دریافت توکن کلودفلر»** را بزنید تا وارد حساب کلودفلر خود شوید.

4. 🟦 در انتهای صفحه باز شده در کلودفلر، روی دکمه آبی‌رنگ `Continue to summary` و سپس `Create Token` کلیک کنید.

5. 🔑 توکن ساخته‌شده را کپی کرده و **در چت ربات ارسال کنید**.

6. ⚡️ پس از تایید توکن، به منوی اصلی برگردید، روی **«🚀 ساخت پنل جدید»** کلیک کرده و اکانت خود را انتخاب کنید تا دیتابیس D1 و پنل شما به‌طور خودکار مستقر شوند.

  

---

  <div  align="center">

<a  href="https://ahura-mazda-panel.ir-netlify.workers.dev/"  target="_blank">

<img src="https://img.shields.io/badge/Launch_Zeus_Panel-Deployer_Site-F38020?style=for-the-badge&logo=cloudflare&logoColor=white" alt="Deploy Zeus" height="40">

</a>

</div>

### 🌐 روش دوم: نصب از طریق سایت Deployer

1. 🌐 وارد **[سایت نصب‌کننده](https://ahura-mazda-panel.ir-netlify.workers.dev/)** شوید.

2. 🟧 بر روی دکمه نارنجی‌رنگ **«دریافت توکن»** کلیک کنید تا به حساب کلودفلر خود هدایت شوید.

3. 🟦 مشابه روش قبل، به انتهای صفحه اسکرول کرده و روی دکمه آبی‌رنگ `Continue to summary` کلیک کنید؛ سپس ساخت توکن را تأیید کرده و کد را کپی کنید.

4. 🔑 به سایت نصب‌کننده بازگردید و توکن را در کادر مربوطه قرار دهید.

5. ⚡️ دکمه سبزرنگ **«ساخت پنل»** را بزنید و منتظر بمانید تا پنل شما به‌طور کامل مستقر شود.

  

---

  
> [!CAUTION]
>  **نکته امنیتی بسیار مهم:** رمز عبور اولیه‌ای که در اولین ورود برای پنل مدیریت خود تنظیم می‌کنید را حتماً در جایی مطمئن یادداشت کنید و آن را فراموش نکنید!

---


  

## 📈 تاریخچه ستاره‌ها (Star History)

  



<a href="https://www.star-history.com/?repos=Ahadakbar1%2Fahura-mazda&type=timeline&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Ahadakbar1/ahura-mazda&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Ahadakbar1/ahura-mazda&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Ahadakbar1/ahura-mazda&type=Date" />
 </picture>
</a>

---

  

## 💰 حمایت مالی (Donate)

  

<p  align="center"> ساخته‌ شده با ❤️</p>

<p  align="center"><a  href="https://donatonion.ir-netlify.workers.dev"><b>https://donatonion.ir-netlify.workers.dev</b></a></p>

<p  align="center"><a  href="https://daramet.com/ir_netlify"><b>https://daramet.com/ir_netlify</b></a> ریالی </p>

<p  align="center"> از حمایت شما برای زنده نگه‌داشتن و توسعه این پروژه منبع‌باز متشکریم 🙏</p>

  

---

  

## ⚖️ حق نشر و اعتبارات

  

این پنل در ابتدا توسط آراد و مورگان نوشته شده بود که بنا به دلایلی ریپازیتوری اصلی حذف شد. نسخه فعلی توسعه‌یافته و بهینه‌شده‌ی همان منطق است.

  

* **سازندگان اولیه:** نسخه پایه و ایده اولیه پروژه متعلق به [AG-Morgan](https://github.com/AG-Morgan) و [aradava](https://github.com/aradava) می‌باشد.

* **توسعه‌دهنده فعلی:** ارتقاء پنل، افزودن قابلیت‌های پیشرفته شبکه، طراحی مجدد رابط کاربری و پیاده‌سازی سیستم دیپلوی خودکار توسط [PANEL_ZEUS](https://t.me/PANEL_ZEUS) انجام شده است.
