...
async function checkAutoRotates(env) {
	...
	try {
		...
		const res = await fetch("https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/ips.txt");
		if (!res.ok) return;
...
async function replaceBrokenProxy(username, env, oldProxy) {
	try {
		...
		if (cachedVipCountries.length === 0 || Date.now() - lastVipCountriesFetch > 3600000) {
			try {
				const ghRes = await fetch("https://api.github.com/repos/Ahadakbar1/ahura-mazda/contents/proxy/proxy_vip", {
		...
		if (upperCountry !== "ALL" && upperCountry !== "UN") {
			sources.push({ url: `https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/proxy/proxy_vip/${upperCountry}.txt`, type: 'repo' });
		}
		for (const fc of fallbackVIPs) {
			if (fc !== upperCountry) {
				sources.push({ url: `https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/proxy/proxy_vip/${fc}.txt`, type: 'repo' });
			}
		}
		if (!isOldProxyVIP) {
			if (upperCountry !== "ALL" && upperCountry !== "UN") {
				sources.push({ url: `https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/proxy/${upperCountry}.txt`, type: 'repo' });
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				const cfHeaders = {
					...
					"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AhuraMazdaPanel/1.0"
				};
				if (!currentAccountId) {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/ahura-mazda.js?t=" + Date.now(), {
					headers: {
				...
				formData.append("ahura-mazda.js", new Blob([newCode], { type: "application/javascript+module" }), "ahura-mazda.js");
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        ...
        <div id="recovery-section" class="hidden">
            ...
            <div class="mb-5 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-md text-xs leading-relaxed text-orange-800 dark:text-orange-300">
                ...
                <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=AhuraMazda-Deployer-Token" target="_blank" class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-900/20 hover:text-green-800 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-900/40 dark:hover:text-green-400 rounded-md font-bold transition shadow-md">
    ...
    <header class="border-b border-gray-200 dark:border-amoled-border bg-white dark:bg-amoled-card px-4 py-4">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="rtl">
                    اهورا مزدا
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://github.com/Ahadakbar1/ahura-mazda" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="GitHub">
                    ...
                    <a href="https://t.me/AHURA_MAZDA_BOT" target="_blank" rel="noopener noreferrer" class="text-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Bot">
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://github.com/IR-NETLIFY/ahura-mazda-relay" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پـروکـسـی شخصی</a>
                            </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://ahura-mazda-panel.ir-netlify.workers.dev/" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div id="token-modal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="token-modal-card" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl p-6 transform transition-all scale-95 opacity-0 duration-200">
            ...
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=AhuraMazda-Deployer-Token" target="_blank" class="flex items-center justify-center gap-2 w-full py-3 bg-[#d94800] hover:bg-[#e35802] text-white font-bold rounded-md text-sm transition duration-300 mb-4 shadow-md shadow-orange-500/20">
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() {
        try {
            const res = await fetch('https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/message.txt?t=' + Date.now());
            if (!res.ok) return;
            const text = await res.text();
            const lines = text.split('\\n');
            ...
            if (window.ahuramazda_global_msg_version !== version) {
                ...
                document.getElementById('global-message-close-btn').onclick = function() {
                    ...
                    window.ahuramazda_global_msg_version = version;
                };
            }
        } catch (err) {}
    }
	...
	try {
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "ahuramazda_full_backup_" + dateStr + ".json");
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/ahura-mazda.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/ips.txt');
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('ahuramazda_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پـنـل تغییر کرد');
            };
            const savedRate = localStorage.getItem('ahuramazda_refresh_rate');
...
function toggleProxySelectorModal(show) { setModalState('proxy-selector-modal', show); }
		async function loadVipCountries() {
			...
			try {
				const res = await fetch('https://api.github.com/repos/Ahadakbar1/ahura-mazda/contents/proxy/proxy_vip');
		...
		async function loadVipProxy() {
			...
			btn.innerText = '...';
			try {
				const url = 'https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/proxy/proxy_vip/' + country + '.txt?t=' + Date.now();
...
async function fetchAndLoadProxy() {
    ...
    try {
		const sources = [
			...
			{ url: 'https://raw.githubusercontent.com/Ahadakbar1/ahura-mazda/refs/heads/main/proxy/' + country.toUpperCase() + '.txt', prefix: '' },
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://github.com/Ahadakbar1/ahura-mazda" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
    ...
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://ahura-mazda-panel.ir-netlify.workers.dev/" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-500 dark:hover:text-amber-300 group">
